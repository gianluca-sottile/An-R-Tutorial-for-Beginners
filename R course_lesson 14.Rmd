...
Notes:
- Since R 4.0, strings are **not** converted to factors by default, but older code often included `stringsAsFactors = FALSE`; keeping it can be useful for portability. [web:691][web:693]
...
The `readxl` package reads both `.xls` and `.xlsx` files. [web:695][web:703]
...
`readxl_example()` lists example spreadsheets shipped with the package. [web:695]
...
Examples (sheet by name or index): [web:695]
...
`cell_rows()` and `cell_cols()` give fine control over the imported rectangle. [web:701][web:695]
...
The `haven` package imports (and exports) common file formats from other statistical software, including SAS, Stata, and SPSS. [web:699][web:672]
...
Tip: `haven::read_spss()` can also choose between SPSS formats based on file extension. [web:699]
