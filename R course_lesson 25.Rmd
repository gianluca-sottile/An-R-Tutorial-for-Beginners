...
Because the initialization is random, k-means can converge to different local optima. For this reason, it is standard practice to use multiple random starts with `nstart` (e.g., 25 or 50) and keep the best solution. [web:454][web:523]
...
Silhouette width measures how well-separated clusters are; higher average silhouette is generally better. [web:481]
...
The gap statistic compares the observed clustering structure to a reference null distribution to help choose \(k\). [web:479][web:488]
...
Silhouette values summarize cohesion and separation; the average silhouette width can be used as an internal quality check. [web:481]
...
K-means can vary depending on initialization. A simple diagnostic is to re-fit the model multiple times (same k) and examine how much the objective function varies. [web:454][web:523]
...
- Choosing \(k\) can be guided by elbow (WSS), silhouette, and gap statistic. [web:473][web:479]
- Use `nstart` (e.g., 25+) and a fixed seed for more stable results. [web:454][web:523]
