...
Stepwise regression is an automated procedure that adds/removes predictors to optimize a criterion (often AIC). In base R, this is done with `stats::step()` which uses AIC by default.
...
The `olsrr` package can fit many subsets and compare them. For example, `ols_step_all_possible()` fits all combinations up to a chosen order.
