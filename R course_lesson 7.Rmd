...
In data analysis you often need to sort rows by one or more variables. In base R, this is commonly done with `order()`, which returns the indices of the sorted order. [web:796]

# order() vs sort()

- `sort(x)` returns the sorted values.
- `order(x)` returns the indices that would sort `x`. [web:796]
...
`dplyr::arrange()` is often easier to read than `order()` when sorting by multiple columns. [web:803]
