...
In base R, the main tool is `merge()`. By default, `merge(x, y)` performs an **inner join** (keeps only matching keys). [web:778]  
You can control outer joins with:
- `all.x = TRUE` → **left join** (keep all rows from `x`) [web:778]
- `all.y = TRUE` → **right join** (keep all rows from `y`) [web:778]
- `all = TRUE` (or both `all.x` and `all.y`) → **full join** (keep all rows from both) [web:778]
...
This returns only producers that appear in both tables (matching `surname`). [web:778]
...
`inner_join()` keeps rows with keys present in both tables and adds columns from `y` to `x`. [web:786]
...
A left join keeps all rows from the left table (`x`) and fills missing matches from the right table (`y`) with `NA`. [web:778]
...
Because “Lucas” has no match in `movies2`, the `title` column becomes `NA` for that row. [web:778]
...
A full join keeps all rows from both tables and uses `NA` where there is no match. [web:778]
